...
int ViconRTClient::getBodyChannelID(const std::string &name) {
	std::vector< BodyChannel >::iterator iBody1;
	std::vector< BodyChannel >::iterator iBody;

	iBody1 = BodyChannels.begin();
	iBody = std::find(BodyChannels.begin(), BodyChannels.end(), name);
	if(iBody != BodyChannels.end())
		return iBody - iBody1;
	return -1;
...
const BodyData &ViconRTClient::getBodyDataByID(int i) {
	return bodyPositions[i];
}

const MarkerData &ViconRTClient::getMarkerData(const std::string &name){
	std::vector<MarkerChannel>::iterator iMarker;
	std::vector<MarkerData>::iterator iMarkerData;
