...
namespace Geometrics
{
	namespace Primitives
	{
		...
		static void reverse_winding(MeshType& mesh, const primitive_metric metric)
		{
		...
		template <typename MeshType>
		static void invert_normal(MeshType& mesh, const primitive_metric metric)
		{
			using namespace DirectX;
			using namespace DirectX::VertexTraits;
			auto& vertices = mesh.vertices;
		...
		template <typename MeshType>
		inline primitive_metric create_cube(MeshType& mesh, float size , bool rhcoords)
		{
			create_box(mesh,DirectX::XMFLOAT3(size, size, size), rhcoords);
		}

		template <typename MeshType>
		inline primitive_metric create_box(MeshType& mesh, const DirectX::XMFLOAT3& size, bool rhcoords, bool invertn)
		{
		...
		//--------------------------------------------------------------------------------------
		template <typename MeshType>
		inline primitive_metric create_sphere(MeshType& mesh, float diameter , size_t tessellation , bool rhcoords , bool invertn )
		{
			static_assert(concept_mesh_type<MeshType>::value, "MeshType must have vertices and indices collection");
			using namespace DirectX;
			auto& vertices = mesh.vertices;
		...
		//--------------------------------------------------------------------------------------
		template <typename MeshType>
		inline primitive_metric create_geo_sphere(MeshType& mesh, float diameter , size_t tessellation , bool rhcoords )
		{
			static_assert(concept_mesh_type<MeshType>::value, "MeshType must have vertices and indices collection");
			using namespace DirectX;
			auto& vertices = mesh.vertices;
		...
		void GeometricPrimitive::CreateCylinder(std::vector<VertexPositionNormalTexture>& vertices, std::vector<uint16_t>& indices, float height, float diameter, size_t tessellation, bool rhcoords)
		{
			...
			for (size_t i = 0; i <= tessellation; i++)
			{
				...
				push_index(indices, (i * 2 + 2) % (stride * 2));
				push_index(indices, i * 2 + 1);

				push_index(indices, i * 2 + 1);
				push_index(indices, (i * 2 + 2) % (stride * 2));
				push_index(indices, (i * 2 + 3) % (stride * 2));
			}
		...
		void GeometricPrimitive::CreateCone(std::vector<VertexPositionNormalTexture>& vertices, std::vector<uint16_t>& indices, float diameter, float height, size_t tessellation, bool rhcoords)
		{
			...
			for (size_t i = 0; i <= tessellation; i++)
			{
				...
				push_index(indices, i * 2);
				push_index(indices, (i * 2 + 3) % (stride * 2));
				push_index(indices, (i * 2 + 1) % (stride * 2));
			}
		...
		void GeometricPrimitive::CreateTorus(std::vector<VertexPositionNormalTexture>& vertices, std::vector<uint16_t>& indices, float diameter, float thickness, size_t tessellation, bool rhcoords)
		{
			...
			for (size_t i = 0; i <= tessellation; i++)
			{
				...
				for (size_t j = 0; j <= tessellation; j++)
				{
					...
					push_index(indices, i * stride + j);
					push_index(indices, i * stride + nextJ);
					push_index(indices, nextI * stride + j);

					push_index(indices, i * stride + nextJ);
					push_index(indices, nextI * stride + nextJ);
					push_index(indices, nextI * stride + j);
				}
			}
